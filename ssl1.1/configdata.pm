...
our %config = (
  ...
  ARFLAGS => [ "r" ],
  CC => "clang",
  CFLAGS => [ "-march=x86-64-v3 -pipe -O2 -DDSLLVM_BUILD -DCSNA_CONSTANT_TIME_CHECK -D_FORTIFY_SOURCE=2 -fstack-protector-strong -fPIE -Wall -Wextra -Wno-unused-parameter -I/home/john/DSMILSystem/toolchains/DSSSL/ssl1.1/include" ],
  CPPDEFINES => [  ],
  CPPFLAGS => [  ],
  CPPINCLUDES => [  ],
  CXX => "clang++",
  ...
  openssldir => "/usr/ssl",
  options => "--prefix=/usr --openssldir=/usr/ssl enable-shared enable-ec_nistp_64_gcc_128 enable-tls1_3 enable-threads no-asan no-buildtest-c++ no-comp no-crypto-mdebug no-crypto-mdebug-backtrace no-devcryptoeng no-egd no-external-tests no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-md2 no-msan no-rc5 no-sctp no-ssl-trace no-ssl3 no-ssl3-method no-ubsan no-unit-test no-weak-ssl-ciphers no-zlib no-zlib-dynamic",
  perl_archname => "x86_64-linux-gnu-thread-multi",
  perl_cmd => "/usr/bin/perl",
  perl_version => "5.40.1",
  perlargv => [ "--prefix=/usr", "--openssldir=/usr/ssl", "shared", "no-weak-ssl-ciphers", "no-ssl3", "no-comp", "enable-ec_nistp_64_gcc_128", "enable-tls1_3", "threads", "linux-x86_64" ],
  perlenv => {
      "AR" => undef,
      "ARFLAGS" => undef,
      "AS" => undef,
