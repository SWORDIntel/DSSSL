...
def run():
    ...
    for required_event in required_events:
        ...
        if count < 100:
            raise Unexpected(
                qlogdir,
                f"unexpectedly low count of event '{required_event}': got {count}",
            )
    ...
    for required_frame_type in required_frame_types:
        sent_count = frame_type_counts.get("transport:packet_sent", {}).get(
            required_frame_type, 0
        )
        if sent_count < 1:
            raise Unexpected(
                qlogdir, f"unexpectedly did not send any '{required_frame_type}' frames"
            )

        received_count = frame_type_counts.get("transport:packet_received", {}).get(
            required_frame_type, 0
        )
        if received_count < 1:
            raise Unexpected(
                qlogdir,
                f"unexpectedly did not receive any '{required_frame_type}' frames",
            )
