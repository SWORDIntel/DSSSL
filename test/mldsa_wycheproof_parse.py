    ...
    for grp in groups:
        ...
        for tst in grp["tests"]:
            if first:
                ...
                if "flags" in tst:
                    if (
                        "IncorrectPrivateKeyLength" in tst["flags"]
                        or "InvalidPrivateKey" in tst["flags"]
                    ):
                        keyOnly = True
                if not keyOnly:
                    print("")
                    print_label(
                        "PrivateKeyRaw", keyname + ":" + alg + ":" + grp["privateKey"]
                    )
    ...
    for grp in groups:
        ...
        for tst in grp["tests"]:
            if first:
                ...
                if "flags" in tst:
                    if (
                        "IncorrectPublicKeyLength" in tst["flags"]
                        or "InvalidPublicKey" in tst["flags"]
                    ):
                        keyOnly = True
                if not keyOnly:
                    print("")
                    print_label(
                        "PublicKeyRaw", keyname + ":" + alg + ":" + grp["publicKey"]
                    )
            testname = name + "_" + str(tst["tcId"])
            print("\n# " + str(tst["tcId"]) + " " + tst["comment"])
